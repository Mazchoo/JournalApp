...
describe('saveEntryToDatabase', () => {
  it('posts the content, the CSRF token and the date slug', () => {
    saveEntryToDatabase({ paragraph0: { text: 'x', height: 220, allow_ai_synthesis: 1, entry: '2024-03-15' } });

    const settings = ajax.last();
    expect(settings.type).toBe('POST');
    expect(settings.url).toBe('/save-entry/');
    expect(settings.data).toEqual({
      content: { paragraph0: { text: 'x', height: 220, allow_ai_synthesis: 1, entry: '2024-03-15' } },
...
describe('save button state', () => {
  it('enableSaveButton makes both the button and the nav link actionable', () => {
    ...
    expect(document.getElementById('save-nav-button')!.classList.contains('disabled')).toBe(false);
  });
  ...
  it('disableSaveButton reverses it', () => {
    ...
    expect(document.getElementById('save-nav-button')!.classList.contains('disabled')).toBe(true);
  });
});

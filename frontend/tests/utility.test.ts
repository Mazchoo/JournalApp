...
describe('getIndexInArr', () => {
  ...
  it('works on live HTMLCollections', () => {
    ...
    expect(getIndexInArr(list.children, document.getElementById('two')!)).toBe(1);
  });
});
...
describe('componentFromTemplate', () => {
  ...
  it('applies the class name when one is given', () => {
    const component = componentFromTemplate('<p></p>', 'div', 'row mt-3 paragraph-entry');
...
describe('changeTooltipTextFromInput', () => {
  it('writes the input value plus a suffix into the tooltip', () => {
    document.body.innerHTML = '<span id="tip"></span><input id="src" value="42">';
    const input = document.getElementById('src') as HTMLInputElement;

    changeTooltipTextFromInput({ target: input } as unknown as Event, '#tip', 'px');
...
describe('reorderOneDivFromAnother', () => {
  it('moves the elements named by the sources into the target, in source order', () => {
    ...
    expect(Array.from(target.children).map((child) => child.id)).toEqual(['second', 'first']);
  });
});

describe('refreshScrollSpies', () => {
  it('refreshes every Bootstrap 5 scrollspy on the page', () => {
    document.body.innerHTML = '<div data-bs-spy="scroll"></div><div data-bs-spy="scroll"></div>';
    const refresh = vi.fn();
...
describe('moveObjectUp / moveObjectDown', () => {
  ...
  it('does not enable the save button when the move is rejected', () => {
    ...
    expect(document.getElementById('btn-save')!.classList.contains('disabled')).toBe(true);
  });

  it('rebuilds the editors of both affected rows', () => {
    const tinymce = window.tinymce as unknown as ReturnType<typeof installFakeTinyMCE>;
...
describe('media type detection', () => {
  ...
  it.each(['a.jpg', 'a.jpeg', 'a.jfif', 'a.PNG'])('treats %s as an image', (name) => {
  ...
  it.each(['notes.txt', 'archive.zip', 'noextension'])('treats %s as unknown', (name) => {
    expect(isImageFile(name)).toBe(false);
    expect(isVideoFile(name)).toBe(false);
    expect(isMeshFile(name)).toBe(false);
  });

...
export class Modal extends PageElement {
  ...
  private child(
    suffix: "title" | "body" | "action" | "close",
  ): HTMLElement | null {
    const modal = this.resolve();
    if (modal === null) return null;
    const child = modal.querySelector<HTMLElement>(
      `#${this.elementId}-${suffix}`,
    );
    if (child === null) {
      console.error(
        `${this.constructor.name}: #${this.elementId}-${suffix} does not exist`,
      );
    }
    return child;
  }
...
function dispatchModalEvent(
  modal: HTMLElement,
  name: "hide.bs.modal" | "hidden.bs.modal",
): void {

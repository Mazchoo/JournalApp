let initializeMeshRenderer = function(canvasElement, inputFile, onComplete) {
    const reader = new FileReader();
    reader.onload = function(e) {
        try {
            renderGLB(canvasElement, e.target.result, onComplete);
        } catch(err) {
...
function renderGLB(canvas, buffer, onComplete) {
    ...
    const fv  = Math.PI / 4;
    const nr  = 0.1, fr = 100;
    const f   = 1 / Math.tan(fv / 2), di = 1 / (nr - fr);
    ...
    function draw() {
        ...
        if (indices) {
            const itype = indices instanceof Uint16Array ? gl.UNSIGNED_SHORT :
                         indices instanceof Uint8Array ? gl.UNSIGNED_BYTE : gl.UNSIGNED_INT;
...
function computeNormals(positions, indices) {
    ...
    for (let t = 0; t < triCount; t++) {
        ...
        const ax = positions[v1]-positions[v0],   ay = positions[v1+1]-positions[v0+1], az = positions[v1+2]-positions[v0+2];
        const bx = positions[v2]-positions[v0],   by = positions[v2+1]-positions[v0+1], bz = positions[v2+2]-positions[v0+2];
        const nx = ay*bz - az*by, ny = az*bx - ax*bz, nz = ax*by - ay*bx;

        norms[v0]+=nx; norms[v0+1]+=ny; norms[v0+2]+=nz;
        norms[v1]+=nx; norms[v1+1]+=ny; norms[v1+2]+=nz;
        norms[v2]+=nx; norms[v2+1]+=ny; norms[v2+2]+=nz;
    }

    for (let i = 0; i < norms.length; i += 3) {
        const len = Math.sqrt(norms[i]*norms[i] + norms[i+1]*norms[i+1] + norms[i+2]*norms[i+2]);
        if (len > 0) { norms[i]/=len; norms[i+1]/=len; norms[i+2]/=len; }
    }
